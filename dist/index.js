...
import { basename as basename2, join as join4, sep as sep2 } from "node:path";
...
function evaluate(rule, sources, allPaths) {
  ...
  return matchingSources.flatMap((file) => {
    ...
    const location = match.anchors === void 0 ? locate(file.source, match.pattern) : locateEligible(file, match.pattern, match.anchors);
    if (location === void 0) return [];
...
function findSelectorTemplateMismatches(rule, file) {
  ...
  for (const document of documents) {
    ...
    for (const [key, expected] of Object.entries(selector)) {
      ...
      const index = (0, import_yaml2.isScalar)(node) && node.range ? node.range[0] : document.range?.[0] ?? 0;
      const templateNode = document.getIn(["spec", "template", "metadata", "labels", key], true);
      const templateIndex = (0, import_yaml2.isScalar)(templateNode) && templateNode.range ? templateNode.range[0] : void 0;
      const selectorLine = lineAtIndex(file.source, index);
      const templateLine = templateIndex === void 0 ? void 0 : lineAtIndex(file.source, templateIndex);
      ...
      const actualDescription = actual === void 0 ? "missing" : JSON.stringify(actual);
      detections.push({
        rule,
        file: file.path,
        ...locateFromIndex(file.source, index),
        ...
        data: {
          ...
          workloadName: typeof workloadName === "string" ? workloadName : void 0,
          selectorKey: key,
...
function changed(file, selectorLine, templateLine) {
  ...
  return templateLine !== void 0 && file.changedLines.has(templateLine);
}
  ...
  for (const anchor of anchors) {
    ...
    for (const match of matchedSource.matchAll(new RegExp(anchor.pattern, flags))) {
      ...
      for (let line = startLine; line <= endLine; line += 1) {
        if (file.changedLines.has(line) && (eligible === void 0 || line < eligible)) eligible = line;
      }
    }
  }
  return eligible;
...
// src/index.ts
function createApp() {
  const app = new Adversary({ name: "kubernetes", version: "0.0.16", review: { maximumFindings: 8 } });
  registerRules(app);
